...
if [ "$BACKGROUND" == "true" ]
then
    echo "nohup $EAP_HOME/bin/$NAME.sh -c $PROFILE-for-$NAME.xml -b 127.0.0.1 -Djboss.socket.binding.port-offset=$off -Djboss.node.name=$NAME 2>&1 &"
    nohup $EAP_HOME/bin/$NAME.sh -c $PROFILE-for-$NAME.xml -b 127.0.0.1 -Djboss.socket.binding.port-offset=$off -Djboss.node.name=$NAME 2>&1 &
    echo $! > $file
else
    echo 'cmd' > $file
    echo "$EAP_HOME/bin/$NAME.sh -c $PROFILE-for-$NAME.xml -b 127.0.0.1 -Djboss.socket.binding.port-offset=$off -Djboss.node.name=$NAME"
    $EAP_HOME/bin/$NAME.sh -c $PROFILE-for-$NAME.xml -b 127.0.0.1 -Djboss.socket.binding.port-offset=$off -Djboss.node.name=$NAME
