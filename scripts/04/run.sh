...
do
    ...
    then
        ...
        if [ "n" == "$DEBUG_SUSPEND"  ]
        then
            ../../scripts/04/runConfiguration.sh --c $name --tp $tmpPrefix --p $profile --d_p $debugPort --o $offset --b true --d_s 'n'
        else
            ../../scripts/04/runConfiguration.sh --c $name --tp $tmpPrefix --p $profile --d_p $debugPort --o $offset --b true --d_s $suspend
        fi
